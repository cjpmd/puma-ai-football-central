        ...
        >
          ...
          <div className="p-4 h-full flex flex-col justify-end relative z-10">
            {topPositions.length > 0 && ( 
              <div className={`absolute left-2 space-y-0.5 ${hasAlerts ? 'top-12' : 'top-3'}`}>
                {topPositions.map((pos) => (
                  <div key={pos.position} className="bg-black/50 text-white text-[10px] font-medium px-1.5 py-0.5 rounded shadow-lg border border-white/30">
                    {pos.display}
                  </div>
                ))}
              </div>
