-- Fix ambiguous event_type column reference in update_player_match_stats function
...
BEGIN
    ...
    -- Calculate match events statistics (FIXED: Removed ambiguous JOIN)
    SELECT 
        COUNT(*) FILTER (WHERE me.event_type = 'goal'),
        COUNT(*) FILTER (WHERE me.event_type = 'assist'),
        COUNT(*) FILTER (WHERE me.event_type = 'save'),
        COUNT(*) FILTER (WHERE me.event_type = 'yellow_card'),
        COUNT(*) FILTER (WHERE me.event_type = 'red_card')
    INTO 
        total_goals_count,
        total_assists_count,
        total_saves_count,
    ...
    AND me.event_id IN (SELECT e.id FROM events e WHERE e.date <= CURRENT_DATE);
    ...
    -- Get recent games with position aggregation AND match events per game (FIXED: No ambiguity)
