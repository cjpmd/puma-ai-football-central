-- Phase 1: Individual Training Plans (ITP) schema
...
-- 2.2 individual_training_sessions
...
-- 2.4 individual_session_completions
...
-- 4.1 Policies: individual_training_plans
-- View: player linked to the plan, assigned coach, permitted team staff (when visibility allows), or global admin
CREATE POLICY IF NOT EXISTS "ITP: view plans" ON public.individual_training_plans
FOR SELECT USING (
...
-- Insert: player can create for self; coach can create for players they staff; admin always
CREATE POLICY IF NOT EXISTS "ITP: player create" ON public.individual_training_plans
...
CREATE POLICY IF NOT EXISTS "ITP: coach create" ON public.individual_training_plans
FOR INSERT WITH CHECK (
  public.is_global_admin_secure()
  OR (
    coach_id = auth.uid()
...
-- Update/Delete: owner player, assigned coach, or admin
CREATE POLICY IF NOT EXISTS "ITP: manage plans" ON public.individual_training_plans
FOR UPDATE USING (
  public.is_global_admin_secure()
  OR coach_id = auth.uid()
  OR EXISTS (
...
CREATE POLICY IF NOT EXISTS "ITP: delete plans" ON public.individual_training_plans
FOR DELETE USING (
  public.is_global_admin_secure()
  OR coach_id = auth.uid()
  OR EXISTS (
...
-- 4.2 Policies: individual_training_sessions (inherit from parent plan)
CREATE POLICY IF NOT EXISTS "ITS: view" ON public.individual_training_sessions
FOR SELECT USING (
  EXISTS (
...
CREATE POLICY IF NOT EXISTS "ITS: manage" ON public.individual_training_sessions
FOR ALL USING (
  EXISTS (
    SELECT 1 FROM public.individual_training_plans p
    WHERE p.id = individual_training_sessions.plan_id AND (
...
-- 4.3 Policies: individual_session_drills (inherit via session -> plan)
CREATE POLICY IF NOT EXISTS "ISD: view" ON public.individual_session_drills
FOR SELECT USING (
  EXISTS (
...
CREATE POLICY IF NOT EXISTS "ISD: manage" ON public.individual_session_drills
FOR ALL USING (
  EXISTS (
    SELECT 1 FROM public.individual_training_sessions s
    JOIN public.individual_training_plans p ON p.id = s.plan_id
...
-- 4.4 Policies: individual_session_completions
-- View allowed if can view parent plan
CREATE POLICY IF NOT EXISTS "ISC: view" ON public.individual_session_completions
FOR SELECT USING (
  EXISTS (
    SELECT 1 FROM public.individual_training_sessions s
    JOIN public.individual_training_plans p ON p.id = s.plan_id
...
-- Insert/Update/Delete allowed for player (owner), assigned coach, or admin
CREATE POLICY IF NOT EXISTS "ISC: manage" ON public.individual_session_completions
FOR ALL USING (
  EXISTS (
    SELECT 1 FROM public.individual_training_sessions s
    JOIN public.individual_training_plans p ON p.id = s.plan_id
